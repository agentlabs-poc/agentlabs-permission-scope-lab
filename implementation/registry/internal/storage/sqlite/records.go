...
func (s *Store) Installed(ctx context.Context, tenantID, slug string) (bool, error) {
	var one int
	err := s.db.QueryRowContext(ctx, `
		SELECT 1 FROM application_registry_l1_records
		 WHERE boundary='tenant' AND tenant_id=? AND key1=? AND key2=? AND key3=?`,
		tenantID, domainKey, installKind, slug).Scan(&one)
	if errors.Is(err, sql.ErrNoRows) {
		return false, nil
	}
	if err != nil {
...
func (s *Store) Installations(ctx context.Context, tenantID, slug string) ([]domain.Installation, error) {
	query := `
		SELECT tenant_id, key3 FROM application_registry_l1_records
	...
	for rows.Next() {
		var tenant, s2 string
		if err := rows.Scan(&tenant, &s2); err != nil {
			return nil, classify(err)
		}
		result = append(result, domain.Installation{TenantID: tenant, Slug: s2})
	}
	return result, classify(rows.Err())
}
