...
const revisionJSON = `{"version":"1","grant_id":"fk3x9r2man0d","revision":2,"parent_grant_id":"fk3x9r2m5iv8","permissions":["hrms:payroll:payslip::read","hrms:payroll:payslip::write"],"scope":{"cert":"C17"}}`
...
func (a *grantRevisionAPI) PublishGrantRevision(_ context.Context, area domain.Area, fixture domain.FixtureContext, source string, raw []byte) (domain.GrantContent, error) {
	...
	return domain.GrantContent{Version: "1", GrantID: "fk3x9r2man0d", Revision: 2, ParentGrantID: "fk3x9r2m5iv8", Permissions: []string{"hrms:payroll:payslip::read", "hrms:payroll:payslip::write"}, Scope: map[string]string{"cert": "C17"}}, nil
}
...
func TestGrantRevisionPublishParsesAndForwardsCanonicalContent(t *testing.T) {
	...
	args := []string{"grant", "publish", "--file", "-", "--support-assignment", "fm5b7t4p5iv8", "--tenant", "acme", "--app", "hrms", "--db", "lab.db", "--fixture-context", "maya-grant-publisher"}
	...
	if api.source != "fm5b7t4p5iv8" || api.fixture.Name != "maya-grant-publisher" || string(api.raw) != revisionJSON || api.area.TenantID() != "acme" || connector.closes != 1 {
...
func TestGrantRevisionRejectsMalformedCommandAndContentBeforePublication(t *testing.T) {
	commands := [][]string{
		...
		{"grant", "publish", "--file", "-", "--support-assignment", "fm5b7t4p5iv8", "--tenant", "acme", "--app", "hrms", "--fixture-context", "maya-grant-publisher"},
		{"grant", "publish", "--file", "-", "--support-assignment", "fm5b7t4p5iv8", "--app", "hrms", "--db", "x", "--fixture-context", "maya-grant-publisher"},
		{"grant", "publish", "fk3x9r2man0d", "--file", "-", "--support-assignment", "fm5b7t4p5iv8", "--tenant", "acme", "--app", "hrms", "--db", "x", "--fixture-context", "maya-grant-publisher"},
	}
...
func TestGrantRevisionRequiresCapabilityAndSuppressesSuccessOnFailure(t *testing.T) {
	args := []string{"grant", "publish", "--file", "-", "--support-assignment", "fm5b7t4p5iv8", "--tenant", "acme", "--app", "hrms", "--db", "x", "--fixture-context", "maya-grant-publisher"}
