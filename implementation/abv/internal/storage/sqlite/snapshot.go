...
func (r *snapshotReader) catalog(applicationID string, catalog *domain.Catalog) error {
	...
	*catalog = domain.Catalog{ApplicationID: applicationID, Generation: generation, Permissions: map[string]domain.PermissionDefinition{}, Scopes: map[string]domain.ScopeDefinition{}, CompatibilityEnabled: compat == 1}
	...
	rows, err := r.conn.QueryContext(r.ctx, `
		SELECT key4,key5,key6,key7,key8,key9,key10,value
		  FROM abv_l1_records
		 WHERE tenant_id='' AND key1='abv' AND key2='permission' AND key3=?
		 ORDER BY key4,key5,key6,key7,key8,key9,key10`, applicationID)
	if err != nil {
		return classify(err)
	}
	for rows.Next() {
		...
		var payload []byte
		if err = rows.Scan(&slots[0], &slots[1], &slots[2], &slots[3], &slots[4], &slots[5], &slots[6], &payload); err != nil {
			rows.Close()
			return classify(err)
		}
		if err = r.add(); err != nil {
	...
	rows, err = r.conn.QueryContext(r.ctx, `
		SELECT key4, value FROM abv_l1_records
		 WHERE tenant_id='' AND key1='abv' AND key2='scope' AND key3=? ORDER BY key4`, applicationID)
	if err != nil {
		return classify(err)
	}
	for rows.Next() {
