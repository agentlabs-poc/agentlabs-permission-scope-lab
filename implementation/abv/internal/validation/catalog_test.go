...
func TestCatalogPermissionRegistrationValidation(t *testing.T) {
	c := catalog()
	valid := domain.PermissionDefinition{ID: "hrms:payroll:payslip::export", Active: true, Boundary: domain.ApplicationBoundary, Namespace: "hrms"}
	for _, tc := range []struct {
		...
		{"valid", valid, nil},
		{"malformed id", domain.PermissionDefinition{ID: "bad*", Active: true, Boundary: domain.ApplicationBoundary}, domain.ErrMalformed},
		{"inactive at registration", domain.PermissionDefinition{ID: valid.ID}, domain.ErrRejected},
		{"already registered", domain.PermissionDefinition{ID: read, Active: true, Boundary: domain.ApplicationBoundary}, domain.ErrConflict},
	} {
...
func TestPermissionMustStartWithItsApplication(t *testing.T) {
	...
	for _, id := range []string{"hrms::read", "hrms:payroll::read", "hrms:payroll:payslip::write"} {
		if err := CheckPermissionRegistration(catalog, domain.PermissionDefinition{ID: id, Active: true, Boundary: domain.ApplicationBoundary}); err != nil {
	...
	for _, id := range []string{"billing:invoice::read", "reporting:ledger:entry::export", "HRMS:payroll::read"} {
		err := CheckPermissionRegistration(catalog, domain.PermissionDefinition{ID: id, Active: true, Boundary: domain.ApplicationBoundary})
...
func TestTheLeadingNounRuleAppliesOnlyAtTheApplicationBoundary(t *testing.T) {
	...
	if err := CheckPermissionRegistrationAt(domain.ApplicationBoundary, catalog, domain.PermissionDefinition{ID: platformPermission, Active: true, Boundary: domain.ApplicationBoundary}); !errors.Is(err, domain.ErrRejected) {
		t.Fatalf("application boundary accepted a foreign namespace: %v", err)
	}
	if err := CheckPermissionRegistrationAt(domain.PlatformBoundary, platform, domain.PermissionDefinition{ID: platformPermission, Active: true, Boundary: domain.ApplicationBoundary}); err != nil {
	...
	if err := CheckPermissionRegistrationAt(domain.PlatformBoundary, catalog, domain.PermissionDefinition{ID: "auth:client::read", Active: true, Boundary: domain.ApplicationBoundary, Namespace: "auth"}); err != nil {
	...
	if err := CheckPermissionRegistrationAt(domain.Boundary("elsewhere"), catalog, domain.PermissionDefinition{ID: "hrms:x::read", Active: true, Boundary: domain.ApplicationBoundary, Namespace: "hrms"}); !errors.Is(err, domain.ErrMalformed) {
