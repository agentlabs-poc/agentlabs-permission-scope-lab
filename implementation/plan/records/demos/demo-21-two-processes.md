...
an ordinary HTTP request into an application endpoint. Nothing about the
...
`89` lines captured, 8 commands. Reproduce with:
...
| The wire carries authority, not decisions | four requests, four identical questions; no request names an endpoint, a method or a resource |
...
permissions (`hrms:payroll:payslip::read` and `…::write`). The run exercises two
of them; the other two are the department listing and the all-certificates
listing, which demonstration 20 already covers at the decision layer, including
the all-values refusal of Q-071.
...
the policy is a compile-time value beside the handler it guards. Whether a
policy should instead be a record — registered with the application, versioned,
and read at boot — is the question this demonstration forces and does not
answer.

**Who the caller is.** `hrms` sends `Authorization: Bearer agent_hrms`, and the
lab's `labAgents` gate compares that string. A deployment issues credentials;
this does not.

**Freshness.** Nothing here carries an epoch, and nothing re-asks. Each request
asks again from scratch, which is correct but says nothing about what a cache
