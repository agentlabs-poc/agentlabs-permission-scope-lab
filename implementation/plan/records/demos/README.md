...
| File | Shows |
|---|---|
| `demo-21-two-processes.svg` · `.md` | **The first capture with two processes.** An ordinary HTTP request reaches an application endpoint, the gate on the client decides, and the handler body runs or does not — 200 inside the boundary, 403 outside it. The same endpoint and the same policy answer differently for two people, which is the selected-versus-inherited asymmetry arriving at the HTTP layer. What crosses the wire is four identical `authority.resolve` questions naming no endpoint, method or resource; what the application links is `authmiddleware` and `authclient` and not `abv`; and with Auth stopped the answer is 503, never 403 |
