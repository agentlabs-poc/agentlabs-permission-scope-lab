...
| File | Shows |
|---|---|
| `demo-19-service-credential.svg` · `.md` | The caller need not be the subject: an application's own credential resolving a human it is not, the same credential returning that human's own answer unchanged, and two refusals — a credential the area is not bound to, and a human actor naming somebody else, which is impersonation rather than delegation |
