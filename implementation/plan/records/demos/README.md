...
| `demo-18-resolve.svg` · `.md` | The read an enforcing client consumes: one human's entitlements as the canonical document, two routes at different lineage depths with the deeper one's scope folded to `dept=FIN` AND `cert=C17`, the same call narrowed to one permission, the form without the explanation that a bearer token would carry, three permissions that resolve to nothing — never registered, registered then retired, registered and simply not held — answered identically because a filter is a narrowing and not an assertion, and what a retirement actually does: the grant narrows to what the catalog still supplies and the route beneath it is untouched (Q-143) |
