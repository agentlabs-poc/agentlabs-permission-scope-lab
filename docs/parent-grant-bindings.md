...
| B09 | ~~G2 disabled; enabled G3 derives from it~~ | ~~G3 is ineffective without a persistent G3 disable write.~~ **Superseded by [Q-132](grant-lifecycle.md): G2 cannot be disabled while G3 depends on it.** |
| B10 | ~~G2 validly enabled; G3 still enabled and otherwise valid~~ | ~~G3 can resume.~~ **Superseded by [Q-132](grant-lifecycle.md): the state it resumes from cannot be reached.** |
| B11 | G3 or its assignment explicitly disabled | Restoring its parent does not enable that disabled record. Under [Q-132](grant-lifecycle.md) the parent could not have been disabled beneath it; the rule that an explicit disable is never undone by another record stands. |
