...
| Re-enable shared grant | Apply grant-wide checks to required affected bindings and current authority. Explicitly disabled assignments remain disabled; their missing support alone is not a grant-enable failure (CP4-P03 reconciliation of Q-101). | Skip a broken enabled assignment, enable the grant only for recipients whose required checks pass, or implicitly enable a disabled assignment. |
