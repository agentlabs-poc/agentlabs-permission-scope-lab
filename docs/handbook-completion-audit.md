...
| HC-05-11 | OPEN | Complete ordinary grant status/validity and create/change/revoke lifecycle rules. | Q-078–080 agree terminal revocation, reversible enable/disable, and create/delete in [grant lifecycle](grant-lifecycle.md). Q-081 proposes administrative status representation. Initial defaults, validity, full transition validation, and operation contracts remain open. |
...
| HC-07-08 | OPEN | Complete grant and role wire schemas, including the agreed lifecycle representation. | Q-081 proposes a status representation in [grant lifecycle](grant-lifecycle.md), not yet approved. [Grant formats](grant-format.md) remain working illustrations; full grant/role schemas and lifecycle representation are unfinished. |
