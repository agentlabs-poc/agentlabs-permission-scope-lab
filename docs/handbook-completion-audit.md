...
| HC-09-09 | OPEN | Define non-HTTP/background-operation integration requirements or explicitly defer them. | Q-075 proposes execution-time authorization for queued work in [background authorization](background-authorization.md), not yet approved. Adapter schemas, trusted job identity/material binding, permission mapping, retries, and running-job behavior remain open; no independent service authority. |
