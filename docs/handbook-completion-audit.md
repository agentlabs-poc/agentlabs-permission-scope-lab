...
| HC-05-10 | OPEN | Specify delegation ceilings, chains, expiry, growth, and reactivation semantics. | Human dependency is agreed. Q-070 proposes explicit renewal for broken delegated authority in [delegation lifecycle](delegation-lifecycle.md), not yet approved; other mechanisms remain open in [grant model](grant-model.md). |
