...
| ENFORCEMENT-004 | PROPOSED | For a same-tenant move changing an authorization boundary, require authority covering both current and proposed boundaries. | Q-068 addresses the source/destination policy deliberately left open in the PUT example. Source-only checks can place data into an unauthorized boundary; destination-only checks can pull data from an unauthorized boundary. One endpoint permission and the existing application-owned enforcement model remain. Grant composition and concurrency are not settled. See operation-enforcement.md. |
...
pass, not excluded from v1. **Q-068 / ENFORCEMENT-004 is proposed**: authorization
of both current and proposed boundaries for a same-tenant move. It is a new branch,
not another result-field question. The tree gives the recommended next impact
pass and the operation chapter records rationale, alternatives, and open details.
