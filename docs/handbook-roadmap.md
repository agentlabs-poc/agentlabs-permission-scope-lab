...
| DECISION-015 | PROPOSED | Reject fields not defined in the supported authorization-result contract rather than ignoring them. | Q-067 distinguishes unknown fields from mixed known-variant fields already rejected by Q-065. Strict rejection exposes typos and contract drift; ignoring extras favors forward compatibility but may conceal unconsumed information. This concerns the result object, not unrelated application data. See decision-results.md. |
