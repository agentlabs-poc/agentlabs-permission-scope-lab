...
| PERMISSION-003 | PROPOSED | A parent permission name does not automatically authorize a deeper permission merely because of a shared namespace prefix. | Q-057 revisits the earlier explicitly open non-inheritance proposal. Avoid silently expanding grants when specialized operations are added; ordinary roles can list both permissions explicitly. Wildcards and other authority mechanisms are not settled by this proposal. See permission-model.md. |
...
`::` before the verb. **Q-057 / PERMISSION-003 is proposed**: no automatic
parent-to-child permission inheritance from name prefixes. Ask this one question;
wildcards, validation details, and decision-result completion remain separate.
