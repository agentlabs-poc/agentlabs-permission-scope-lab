...
| ENFORCEMENT-008 | PROPOSED | Preserve evaluated application boundaries through the protected data operation; a concurrent boundary change must not turn an earlier allow into an out-of-boundary effect. | Q-074 uses C-17 moving from Finance to Engineering between authorization and update. Stop the original attempt if its required boundary no longer holds; no ID-only fallback or silent material substitution. Mechanisms, conflict reporting, retries, and in-flight Auth authority changes remain open. See concurrent-enforcement.md. |
...
checks before effects, without automatic partial execution. Next is
**Q-074 / ENFORCEMENT-008, proposed:** preserve evaluated application boundaries
through use despite concurrent record changes. Keep transaction mechanisms and
detailed schemas open rather than drilling further here.
