...
| DECISION-011 | PROPOSED | Minimal deny representation: version "1", decision "deny", error_code, error_message, and error_message_reason. | Q-063 assembles agreed fields without introducing new names; actual established cause remains required. The example code NO_AUTHORIZING_GRANT is illustrative, not a finalized catalogue. No supporting-grant list or rejected-candidate trace is proposed for this minimum. Evaluation-error representation remains separate. See decision-results.md. |
...
DECISION-011 is proposed**: minimal deny JSON with version, decision, and the
agreed error code/two messages. Ask one
