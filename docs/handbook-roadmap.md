...
| SCOPE-007 | PROPOSED | Minimal v1 draft: required flat JSON object of defined boundary keys and non-empty string values, concrete references or the reserved $self human reference where supported; entries combine with AND. Reject unknown/duplicate keys and unsupported forms rather than dropping restrictions. No arrays, nested objects, OR/wildcard operators in this draft. | User now requests canonical format. scope-model.md presents the draft and Q-034 asks whether explicit {} means tenant-wide while missing/null stays invalid. Draft constraints, exact token syntax, and empty-object behavior await approval; meaning and AND combination are already agreed separately. |
...
| Q-034 | OPEN | In the minimal key-value format, should explicit {} mean tenant-wide reach for the grant's permissions, while missing or null scope is invalid? | SCOPE-007 draft recommends this distinction to preserve implicit tenant without adding a field. Broadening risk and the alternative of an explicit tenant-wide form are documented; no default from missing/invalid scope to {} is permitted in the draft. |

## Resume here

- Current position: [discussion tree](discussion-tree.md), stage 6 →
  SCOPE-006/008 boundary semantics are agreed; SCOPE-007 minimal format is drafted,
  with Q-034's explicit empty-object choice pending. Q-033 approves CONTRACT-006's
