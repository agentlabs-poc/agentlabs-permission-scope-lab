...
| PERMISSION-004 | PROPOSED | Exclude wildcard permission names from v1; use explicit registered permissions, optionally bundled in roles. | Q-058 recommends avoiding new matching-depth and future-permission expansion rules in v1. The alternative one-segment wildcard remains preserved from the earlier Markdown. This is not approved and does not decide aliases or change live-role semantics. See permission-model.md. |
...
is proposed**: exclude wildcard permission names from v1. Ask this one question;
aliases, validation details, and decision-result completion remain separate.
