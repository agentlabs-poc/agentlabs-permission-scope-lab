...
| ENFORCEMENT-009 | PROPOSED | Queued work obtains authorization for the protected operation at execution time; submission's stored allow is not a durable execution grant. | Q-075 uses a queued Finance export whose sole supporting grant is revoked before the worker starts. A new evaluation preserves current human-dependent authority; accepted jobs may therefore fail to execute. No new job schema, independent worker authority, retry policy, or prepared state is introduced. See background-authorization.md. |
...
through use despite concurrent record changes. Next is
**Q-075 / ENFORCEMENT-009, proposed:** queued work needs execution-time authorization,
not reuse of submission's allow. Keep transaction mechanisms, in-flight authority
changes, and detailed adapter schemas open rather than drilling further here.
