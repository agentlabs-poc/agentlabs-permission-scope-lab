...
| GRANT-007 | REVISED PROPOSAL — NOT APPROVED | Use one administrative status field with enabled or disabled; delete removes the grant, effective validity remains derived. | Original Q-081 proposed three values including revoked and was never approved. Q-082 supersedes that variant; revised Q-081 proposes only enabled/disabled. Original JSON and rationale retained as history in grant-lifecycle.md. Initial defaults and the complete schema remain open. |
...
Next is **Q-081 revised / GRANT-007, proposed**: status enabled/disabled only;
the original three-state variant was never approved. Effective validity, scope
satisfaction, and dependent access are not equated with enabled status.
