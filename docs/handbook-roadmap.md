...
| DECISION-013 | PROPOSED | Reject responses mixing known variant-specific fields from allow, deny, and evaluation-error shapes. | Q-065 recommends unambiguous interpretation across consumers. An allow containing error_code must not authorize execution merely because a caller ignores the conflicting field. Reject as malformed, not as a completed policy denial. Unknown extensions, exact value validation, and code compatibility remain open. See decision-results.md. |
...
proposed**: reject mixtures of known result-variant fields. Ask one
