...
| PERMISSION-005 | PROPOSED | Exclude permission aliases from v1; registered permission identifiers are not interchangeable through alias mappings. | Q-059 recommends keeping references unambiguous across grants, endpoints, registration, and audit. Explicit aliases could aid compatibility but require extra equivalence/migration rules. UI display labels are unaffected, and separately registered names remain distinct. See permission-model.md. |
