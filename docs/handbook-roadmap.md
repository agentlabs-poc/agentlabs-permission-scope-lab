...
| AUDIT-001 | OUTSIDE HANDBOOK SCOPE — PROPOSAL NOT ADOPTED | Audit policy and system design belong to another layer; do not require their completion for this handbook. | User answered Q-076: “This itself is different layer question. handbook as no scope for this.” The earlier mandatory-audit proposal is retained in authority-change-audit.md as history, not an adopted rule or parked task. Authorization-result evidence and authorization of administrative changes remain in scope. HC-09-08 is excluded, not completed. |
| GROUP-005 | AGREED — NOT SUPPORTED | Resolve group grants through direct human-to-group memberships only; nested authorization groups are not supported. | User answered Q-077 “nested group not supported.” Direct membership avoids recursive traversal, cycles, and transitive dependency rules, at the cost of maintaining multiple explicit memberships. Human-only membership stays agreed; no promise of later nesting support. Rationale and original proposal status retained in groups-and-membership.md. Membership and sync lifecycle remain open. |
| GRANT-004 | AGREED PERMANENCE; SEPARATE OPERATION SUPERSEDED | Permanent withdrawal remains terminal for the grant, now through delete under Q-082 rather than a separate revoke operation. | Q-078 was approved; Q-082 later consolidates revoke/delete. Restoring access still requires a newly authorized grant, unlike reversible disable or dependent inactivity. Original decision and rationale retained in grant-lifecycle.md. |
| GRANT-005 | AGREED — USER ADDITION | Support reversible grant disable/enable for operational efficiency; enable restores eligibility, not unconditional authority or a revoked grant. | Q-079 records the user's explicit addition. Same-grant operational suspension avoids creating replacements for temporary pauses; all scope, validity, recipient, and proxy constraints still apply. Administrative authorization remains required. See grant-lifecycle.md. |
...
| ROLE-001 | AGREED | A role is a named reusable permission bundle. A grant references either such a role or an explicit permission set and binds it to a recipient, scope, validity, and conditions. A role definition supplies capabilities; the grant supplies their assigned reach. Group is the collection of human recipients, role is the collection of capabilities, and grant binds recipient to scoped authority. | User answered Q-013: "makes sense." GRANT-EX-004 illustrates this. Role-change behavior was settled by ROLE-002, now deprecated and replaced by ROLE-003 / Q-089-B. Role templates/defaults, compatibility, and remaining revision mechanics stay open. |
| ROLE-002 | DEPRECATED BY ROLE-003 / Q-089-B | A role-referencing grant uses the role's current permission definition. Authorized role edits therefore change capabilities available through existing referencing grants while preserving each grant's scope, validity, and conditions. Unrelated direct grants remain separate. | Historical decision preserved: user answered Q-014 “yes.” Its automatic live-update meaning is now superseded by immutable revisions and explicit authorized grant adoption under Q-089-B. It is no longer the current rule. |
| RESOLUTION-003 | AGREED AS UPDATED BY ROLE-003 | Expand the explicitly adopted role revision into its permission set, producing the same permission-set view used to evaluate explicit permissions. Preserve the original grant identity, scope, recipient, validity, conditions, dependencies, and adopted role provenance. Expansion is a computed view of the same grant, not a new assignment, and does not merge unrelated grants. | Q-089-B supersedes the former “current permission set” source: it meant the live role definition, now deprecated. The original dependent-view meaning survives. GRANT-EX-005 is retained as historical illustration; exact resolved transport fields remain open. |
| ROLE-003 | AGREED | Published role revisions are immutable. Each role-based grant selects role_id and role_revision explicitly. Publishing a revision does not change existing grants; adoption is an authorized grant change with validation of the resulting assignment and boundaries. Resolution uses the adopted revision, never silently latest. | User proposed revision/adoption separation and instructed “record this commit and push” after Q-089-B's concrete blocks. Rationale: separate authoring reusable definitions from changing customer authority. Auth's own APIs obey the framework. Removing permissions in a newer revision does not withdraw them from older adopted revisions. Full lifecycle, schemas, migration, and concurrency remain open. See role-revisions.md. |
...
not reuse of submission's allow. **Q-076 / AUDIT-001 is outside handbook scope**:
the user places audit in another layer and the proposal is not adopted. Older
roadmap references to audit-system deliverables are superseded by this scope
decision, not pending work. **Q-077 / GROUP-005 is agreed:** nested authorization
groups are not supported; membership is direct human-to-group.
**Q-082 / GRANT-008 is agreed:** delete is the single permanent-removal operation.
...
**Q-088 is agreed**; the user answered “yes, agreed”:
bootstrap creates minimal ordinary grants that may remain after setup. Later
self-assignment is explicit and must fit existing administrative authority.
“Bypass” meant authority without an applicable grant, not retaining seed grants.
See [bootstrap authority](bootstrap-authority.md) for examples and rationale.
**Q-089-B / ROLE-003 is agreed.** The user requested recording, commit, and push
after proposing immutable role revisions with explicit, boundary-validated grant
adoption. This replaces automatic live references, not merely their validation
mechanism. The original Q-089 alternatives remain undecided history with their
premise superseded; see [original discussion](role-change-authority.md).
The user also clarified that Auth's own APIs comply with this framework; role
publication does not silently grant customer/application authority. See
[approved revision/grant blocks, rationale, and withdrawal caveat](role-revisions.md).
Full endpoint schemas, revision lifecycle, migration, and concurrency remain open.
