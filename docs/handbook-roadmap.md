...
| FRESHNESS-001 | PROPOSED | After Auth confirms a grant's revocation, authorization checks started afterward cannot use that grant through a stale cache. | Q-069 recommends no stale-cache grace period for new checks. The alternative is documented bounded staleness, trading prompt revocation for less coordination. Other valid grants remain usable; inability to establish freshness cannot justify stale allow. Cache protocol, in-flight operations, and change propagation beyond grant revocation remain open. See authority-freshness.md. |
...
to **Q-069 / FRESHNESS-001, proposed**: no stale-cache use of a revoked grant by
