...
| ENFORCEMENT-006 | PROPOSED | Distinguish an explicitly bounded collection request from a broader request; Finance-only authority can cover an explicit Finance list but cannot silently narrow an all-departments list. | Q-072 checks the consequence of Q-071 with two concrete routes. Request-bound query constraints remain application responsibility, not grant-derived filtering. Endpoint contracts, not today's stored rows, define the requested reach. See collection-enforcement.md; not yet approved. |
...
rejected the endpoint complexity of grant-derived subset filtering. Next is
**Q-072 / ENFORCEMENT-006, proposed**: check the distinction between an explicitly
Finance-bounded request and an all-departments request. Keep restoration mechanics
and detailed schema work open rather than drilling further here.
