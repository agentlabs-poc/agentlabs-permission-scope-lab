...
| ENFORCEMENT-005 | PROPOSED | An ordinary scoped collection read returns the authorized subset rather than denying solely because out-of-scope records exist in the broader collection. | Q-071 recommends usable scoped listing without tenant-wide authority. Finance listing permission supplies Finance results, not Engineering data; no applicable authority is not merely an empty authorized collection. Actual data-path enforcement remains mandatory. Pagination/counts, exports, and bulk writes remain open. See collection-enforcement.md. |
...
for this case. Move horizontally to **Q-071 / ENFORCEMENT-005, proposed**:
authorized-subset behavior for ordinary scoped collection reads. Keep restoration
mechanics and detailed schema work open rather than drilling further here.
