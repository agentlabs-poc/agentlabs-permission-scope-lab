...
| ENFORCEMENT-007 | PROPOSED | For one bulk request, establish authorization and required boundary checks for the complete batch before effects; an uncovered item blocks the whole operation rather than triggering partial execution. | Q-073 uses a Finance deletion batch containing an Engineering certificate. One authorization outcome avoids surprise partial changes, at the cost of one uncovered item blocking the batch. This is not a database rollback guarantee; concurrency, transactions, retries, representation, and error mapping remain open. See bulk-enforcement.md. |
...
Next is **Q-073 / ENFORCEMENT-007, proposed**: complete-batch authorization and
boundary checks before effects, without automatic partial execution. Keep
transaction mechanisms and detailed schemas open rather than drilling further here.
