...
| DECISION-010 | PROPOSED | Minimal allow representation: version "1", decision "allow", and grant_ids containing supporting grant identifiers. | Q-062 proposes concrete names for the already-agreed decision and references, without returning full grants or scope. An array is simpler than reference objects absent additional required metadata. Full schema, historical evidence, correlation, and other variants remain open. See decision-results.md. |
...
proposed**: minimal allow JSON with version, decision, and grant_ids. Ask one
