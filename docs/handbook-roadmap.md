...
| DELEGATION-003 | PROPOSED | After a required supporting relationship breaks, restoring the human's access does not automatically reactivate the affected delegation; require explicit authorized renewal. | Q-070 recommends intentional restoration rather than unnoticed recovery of old automation. Automatic reactivation is simpler but can revive forgotten delegations. Unrelated supported access remains unaffected; lookup failures do not prove a relationship broke. Renewal authority, representation, and timing remain open. See delegation-lifecycle.md. |
...
Next is **Q-070 / DELEGATION-003, proposed**: explicit authorized renewal instead
of automatic reactivation of a delegation after its required support broke.
The existing invalidation rule is settled; this asks about later restoration.
