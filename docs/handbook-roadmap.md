...
| DECISION-012 | PROPOSED | Minimal evaluation-error representation: version "1" and the three agreed error fields, without decision. | Q-064 recommends reflecting absence of a completed decision without adding a third decision value or another status field. A timeout with no sufficient valid authority is the example; code spelling is illustrative. Missing decision alone does not validate arbitrary malformed output, and protected execution remains blocked. Full validation and transport remain open. See decision-results.md. |
...
agreed error code/two messages. **Q-064 / DECISION-012 is proposed**: evaluation-error
JSON using the same error fields without decision. Ask one
