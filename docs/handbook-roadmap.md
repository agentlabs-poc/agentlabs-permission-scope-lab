...
| DECISION-008 | PROPOSED | Return supporting-grant references with an allow result to the server-side endpoint. | Q-060 extends the existing provenance requirement into the returned result, so the endpoint/audit path need not reconstruct why access was authorized. It does not return all grants, replace scope enforcement, select exact fields, or mandate UI disclosure. See decision-results.md. |
...
the denominator. **Q-060 / DECISION-008 is proposed**: return supporting-grant
references with an allow result. This is the return to decision results; ask
one question at a time, with rationale in the chapter.
