...
| PERMISSION-002 | PROPOSED | Retain the earlier namespaced-noun::verb convention as canonical permission naming. | User asked to retain the earlier permission explanation. The operation/reach definition is already agreed, but PERMISSION-001 explicitly left grammar open; Q-056 asks the focused adoption question. permission-model.md restores the detailed explanation and rationale without reviving deprecated vocabulary, scopes, or wildcard proposals. |
