...
|---|---|---|---|
| SCOPE-003 | PROPOSED | Treat scope as the owner of target-selection semantics and grant as a binding that uses scope without defining its domain meaning. Application-defined concepts such as department are optional and resource-compatible; shared concepts such as self require explicit application/resource relationships. | User's Q-026 response proposes scope ownership, standard/custom distinctions, and reconsidering the canonical arrangement if scope is a selector. Q-027 tests conceptual separation, not a new entity, field, fixed catalog, general expression grammar, or mutable scope-reference model. See scope-model.md. |
...
| Q-027 | OPEN | Should scope own the definition of target selection, while a grant only binds its use to recipient and permissions, with shared and application-defined meanings validated through the scope contract? | SCOPE-003 proposed; semantic ownership does not mandate independent storage or finalized selector syntax. |

## Resume here

- Current position: [discussion tree](discussion-tree.md), stage 6 →
  SCOPE-003 / Q-027: scope-owned selection, grant binding, and shared versus
  application-defined meanings. SCOPE-002 representation remains proposed.
