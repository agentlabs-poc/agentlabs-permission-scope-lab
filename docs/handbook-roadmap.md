...
| GRANT-004 | AGREED | Explicit revocation is terminal for that grant; restoring access requires a newly authorized grant rather than reactivation of the old grant identity. | User answered Q-078 “yes,” then separately added reversible disable/enable and create/delete. Stable revocation meaning remains distinct from suspension and Q-070's dependent inactivity. Rationale and previous proposal status retained in grant-lifecycle.md. |
...
| GRANT-006 | AGREED — USER ADDITION | Support authorized grant creation and deletion alongside enable, disable, and revoke. | Q-080 records the user's explicit create/delete addition. Create establishes a new binding; delete removes usable authority, without prescribing physical erase or retention. Neither operation bypasses administration or revives revoked identity. See grant-lifecycle.md. |
| GRANT-007 | PROPOSED | Use one administrative status field with enabled, disabled, and revoked; create/delete remain operations, effective validity remains derived. | Q-081 reuses the existing illustrative status field, proposing enabled instead of active to avoid implying a request is allowed. Earlier active examples are retained, not silently canonicalized. Initial defaults, full transitions, validity, and the complete schema remain open. See grant-lifecycle.md. |
...
groups are not supported; membership is direct human-to-group. Next is
**Q-078 / GRANT-004 is agreed:** explicit revocation is terminal for that grant.
The user added **Q-079 / GRANT-005, agreed:** reversible enable/disable, and
**Q-080 / GRANT-006, agreed:** create/delete. Next is **Q-081 / GRANT-007, proposed**:
one administrative status field with enabled/disabled/revoked. Effective validity,
scope satisfaction, and dependent access are not equated with enabled status.
