...
| DECISION-014 | PROPOSED | Allow requires a non-empty grant_ids array of non-empty supporting-grant string identifiers. | Q-066 recommends preserving Q-060's required evidence even when persistent logging is disabled. Empty references cannot stand for valid grant-backed authority; tenant-wide scope still has a supporting grant. Exact ID grammar, provenance detail, and route selection remain open. See decision-results.md. |
...
is proposed**: require at least one supporting grant ID in an allow result. Ask one
