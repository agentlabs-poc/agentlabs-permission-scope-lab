...
| DECISION-009 | PROPOSED | Return evaluated boundary information with allow, associated with its supporting authority routes. | Q-061 recommends making enforcement reach explicit alongside traceability, instead of requiring a second grant lookup or relying only on separate shared context. Preserve route bindings and mandatory limits; no merged broader scope, prepared state, new authority, or exact fields adopted. See decision-results.md. |
...
**Q-061 / DECISION-009 is proposed**: return evaluated boundaries alongside those
references for enforcement. This is the return to decision results; ask one
