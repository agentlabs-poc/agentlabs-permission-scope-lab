...
| SCOPE-007 | PROPOSED | Represent a scope with key-value boundary selections, such as dept:dept-1 and user:$self. Keys denote defined boundary meanings; values select them. | User proposed this while "just thinking". Meaning and AND combination are agreed separately, but exact keys, allowed value forms, $self spelling, missing/empty scope, and validation are not finalized. Examples are not a final wire schema. |
...
- Current position: [discussion tree](discussion-tree.md), stage 6 →
  SCOPE-006/008 boundary semantics are agreed; SCOPE-007 key-value grammar remains
  proposed. Q-033 approves CONTRACT-006's endpoint-owned gate; INPUT-001 captures
